...
@php
	...
	// HTML or TEXT タブ
	if ( $mailmag->mail_text_type ){
		$mail_text_type = $mailmag->mail_text_type;
	}
	else {
		$mail_text_type = 'html';
	}
...
<ul class="nav nav-tabs">
	<input type="hidden" id="mail_text_type" name="mail_text_type" value="{{ $mail_text_type }}">
    <li class="nav-item" style="width:15%;text-align:center;">
      <a href="#html-set" id ='html-button' class="nav-link active" data-toggle="tab">HTML</a>
    </li>
    <li class="nav-item" style="width:15%;text-align:center;">
      <a href="#text-set" id ='text-button' class="nav-link" data-toggle="tab">TEXT</a>
    </li>
  </ul>
<div class="tab-content">
    <div id="html-set" class="tab-pane active">
      <textarea id="html" name= {{ $column_name }} class="summernote form-control">@if(old($column_name)){{ old($column_name) }}@else{{$column_value}}@endif</textarea>
    </div>
    <div id="text-set" class="tab-pane">
      <textarea id="text" style= "position: relative;top: 0px;width: 100%;">@if(old($column_name)){{ old($column_name) }}@else{{$column_value}}@endif</textarea>
    </div>
</div>
...
$(function() {
	...
	var type_name = {!! "'".$mailmag->mail_text_type."'" !!};

	if (type_name == 'text') {
		$('a#text-button').tab('show');
	} else {
		$('a#html-button').tab('show');
	}
	...
	$('.nav-tabs a').click(function (e) {
		...
		if ($('a.nav-link.active').html() == "HTML") {
			$('#mail_text_type').val("html");
			$('textarea#html').attr('name', column_name);
			$('textarea#text').attr('name', '');
		} else {
			$('#mail_text_type').val("text");
			$('textarea#html').attr('name', '');
			$('textarea#text').attr('name', column_name);
		}
	})
});
...
*/
	...
	*/
    ...
    $( "#html" ).summernote({
		...
		// lang   : "ja-JP" ,
        lang: 'ja-JP', // Change to your chosen language
